...
Given(
  "user logs to site with {string} and {string}",
  { timeout: 60000 },
...
Then("product is present in orderhistory page", async function () {
  ...
  await expect(
    this.page.getByRole("heading", { name: "Your Orders" })
  ).toBeVisible();
  ...
  await expect(this.page.locator(".email-title")).toContainText(
    " order summary "
  );
